...
const renderOutcrop = (feature: LunarTerrainFeature) => {
  const x = feature.x * 100
  const y = feature.y * 100
  const path = rockBodies[feature.variant]
  const mask = feature.neighborMask ?? 0
  const bridges = [
    mask & 1 ? '<rect x="33" y="-4" width="38" height="19"/>' : '',
    mask & 2 ? '<rect x="84" y="32" width="20" height="40"/>' : '',
    mask & 4 ? '<rect x="31" y="84" width="40" height="20"/>' : '',
    mask & 8 ? '<rect x="-4" y="31" width="20" height="42"/>' : '',
  ].join('')
  return `<g transform="translate(${x} ${y})"><g fill="#20241f" opacity=".7" transform="translate(5 7)"><path d="${path}"/>${bridges}</g><g fill="url(#terrain-rock)" stroke="#252a24" stroke-width="2"><path d="${path}"/>${bridges}</g><path d="M17 17 67 10 82 30 51 44 20 36Z" fill="#e2d9be" opacity=".11"/><path d="m62 65-18 17m27-10-10 12" fill="none" stroke="#20241f" stroke-opacity=".34" stroke-width="3"/></g>`
}
...
const terrainSvg = (terrain: LunarTerrain, width: number, height: number) => {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${viewWidth} ${viewHeight}" preserveAspectRatio="none"><defs><linearGradient id="terrain-rock" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#747569"/><stop offset=".34" stop-color="#585c52"/><stop offset=".72" stop-color="#484d44"/><stop offset="1" stop-color="#30352f"/></linearGradient><linearGradient id="terrain-stone" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#797a6d"/><stop offset="1" stop-color="#3f443c"/></linearGradient><radialGradient id="terrain-crater" cx="38%" cy="34%" r="68%"><stop offset="0" stop-color="#2f332d"/><stop offset=".5" stop-color="#3d4139"/><stop offset=".68" stop-color="#7a796a"/><stop offset=".82" stop-color="#52564c" stop-opacity=".35"/><stop offset="1" stop-color="#52564c" stop-opacity="0"/></radialGradient><pattern id="terrain-scuff" width="23" height="17" patternUnits="userSpaceOnUse"><path d="M0 15 23 8" stroke="#d8cfb1" stroke-opacity=".08" stroke-width="2"/><circle cx="7" cy="4" r="1.5" fill="#22261f" opacity=".2"/></pattern></defs><g fill="none" stroke-linecap="round"><path d="${northRidge}" stroke="#e1d9bd" stroke-opacity=".08" stroke-width="18"/><path d="${southRidge}" stroke="#20241f" stroke-opacity=".22" stroke-width="24"/><path d="M${viewWidth * 0.82} 0l-34 145 55 96-48 132 43 118-58 148 31 142" stroke="#232720" stroke-dasharray="34 12 8 16" stroke-opacity=".2" stroke-width="4"/></g>${terrain.regolith.map(renderRegolith).join('')}${terrain.craters.map(renderCrater).join('')}${terrain.tracks.map(renderTrack).join('')}${terrain.outcrops.map(renderOutcrop).join('')}${terrain.debris.map(renderDebris).join('')}</svg>`
}
